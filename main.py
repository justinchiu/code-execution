...
def compile_cpp(code: str, output_path: str) -> tuple[bool, str, float]:
    ...
    try:
        ...
        compile_process = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=10  # 10 seconds timeout for compilation
        )
...
async def execute_code(request: CodeExecutionRequest) -> CodeExecutionResponse:
    if request.language.lower() != "cpp":
        raise HTTPException(status_code=400, detail=f"Language '{request.language}' is not supported. Currently only 'cpp' is supported.")
    ...
    try:
        # Compile the code
        compile_success, compile_error, compilation_time = compile_cpp(request.code, executable_path)
        
        if not compile_success:
            raise HTTPException(status_code=400, detail=f"Compilation error: {compile_error}")
        ...
        for test_case in request.stdin_stdout:
            actual_stdout, execution_time = execute_program(executable_path, test_case.stdin)
        ...
        avg_execution_time = total_execution_time / len(request.stdin_stdout) if request.stdin_stdout else 0
